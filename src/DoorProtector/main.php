...
class main extends PluginBase implements Listener{
								...
								if($this->isinviter($doorplace,$name)){
									...
									if($this->setting->get("beginnermode")){
										if($this->history->get($name) < 4){
											$player->sendMessage("§a【運営】 §cそこはあなたの家ではありません");
											$player->sendMessage("§a【運営】 §cまた不法侵入すると、処罰の対象となります");
										}else{
					...
					switch($this->action[$name][0]){
						case "info":
							if($this->doordata->exists($doorplace)){
								...
								$player->sendMessage("§a【運営】 §fこの扉は誰も保護していないようです");
								$event->setCancelled();
								unset($this->action[$name]);
							}
						break;
							...
							if($this->doordata->exists($doorplace)){
								if($player->isOp()){
									if(!isset($a["inviter1"])){
										...
										$this->doordata->save();
										$player->sendMessage("§a【運営】 §f".$this->action[$name][1] ."を招待しました");
										$event->setCancelled();
										unset($this->action[$name]);
										break;
									}
									...
									if(isset($a["inviter5"])){
										$player->sendMessage("§a【運営】 §cこれ以上招待することはできません");
										$event->setCancelled();
										unset($this->action[$name]);
										break;
									}
									...
									if($master[0] == $ip || $master[1] == $name){
										...
										$this->doordata->save();
										$player->sendMessage("§a【運営】 §f".$this->action[$name][1] ."を招待ました");
										$event->setCancelled();
										unset($this->action[$name]);
										break;
										}
										...
										$player->sendMessage("§a【運営】 §cあなたはこのドアのオーナーではありません");
										$event->setCancelled();
										unset($this->action[$name]);
									}
								}
						...
						case "public":
							if($this->doordata->exists($doorplace)){
								...
								$this->doordata->save();
								$player->sendMessage("§a【運営】 §fこのドアを公共化しました");
								$event->setCancelled();
								unset($this->action[$name]);
								}else{
									$master = explode(',',$this->getowner($doorplace));
								...
								if(
										...
										$player->sendMessage("§a【運営】 §cこのドアは既に保護済みです");
										$event->setCancelled();
										unset($this->action[$name]);
									}
								}
						...
						case "protect":
							if($this->doordata->exists($doorplace)){
								...
								$this->doordata->save();
								$player->sendMessage("§a【運営】 §fこのドアを保護しました");
								$event->setCancelled();
								unset($this->action[$name]);
							}
	...
	public function onCommand(CommandSender $sender,Command $command,string $label,array $args) :bool
	{
			...
			case "dp":
				if($sender instanceof Player) {
					...
					switch($args[0]){
						...
						case "info":
						if($sender->isOp()){
							$sender->sendMessage("§a【運営】 §f情報を確認したいドアをタップしてください");
							$this->action[$name][0] = "info";
				        }else{
				            $sender->sendMeaaage("§cこのコマンドを実行する権限ががありません");
				        }
						break;
						...
						case "invite":
							if(!isset($args[1])){
								$sender->sendMessage("§a【運営】 §cアクセスを許可する人の名前が記入されていません");
							}else{
								$sender->sendMessage("§a【運営】 §fアクセスを許可するドアをタップしてください(オーナー、op)のみ");
						...
						case "p":
						case "protect":
							$sender->sendMessage("§a【運営】 §f保護したいドアをタップしてください");
							$this->action[$name][0] = "protect";
						break;
						
						case "public":
							if(!isset($args[1])){
								$sender->sendMessage("§a【運営】 §cメッセージが入力されていません");
							}else{
								$sender->sendMessage("§a【運営】 §f公共化したいドアをタップして下さい");
						...
						case "cancel":
							$sender->sendMessage("§a【運営】 §f選択をキャンセルしました");
							unset($this->action[$name]);
						break;
						
						case "check":
							if(!isset($args[1])){
								$sender->sendMessage("§a【運営】 §cプレイヤー名が入力されていません");
							}else{
								if($this->history->exists($args[1])){
									$sender->sendMessage("§a【運営】 §f" .$args[1] ."が不法侵入した回数 : " .$this->history->get($args[1]) ."回");
								}else{
									$sender->sendMessage("§a【運営】 §f" .$args[1] ."は、まだ不法侵入したことがありません");
								}
							}
						break;
						
						default:
							$sender->sendMessage("§a【運営】 §cそのサブコマンドは存在しません");
						break;
					}
					}
			}else{
			...
			case "setdp":
				if(!isset($args[0])){
					...
					switch($args[0]){
						case "penalty":
						if(!isset($args[1])){
							$sender->sendMessage("§a【運営】 §cペナルティーを 0,1,2,3 の中から選択してください");
						}else{
							switch($args[1]){
								case "0":
								$this->setting->set("penalty","0");
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ペナルティーが0に変更されました");
								break;
								
								case "1":
								$this->setting->set("penalty","1");
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ペナルティーが1に変更されました");
								break;
								
								case "2":
								$this->setting->set("penalty","2");
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ペナルティーが2に変更されました");
								break;
								
								case "3":
								$this->setting->set("penalty","3");
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ペナルティーが3に変更されました");
								break;
								
								default:
								$sender->sendMessage("§a【運営】 §f0,1,2,3 から選んで下さい");
						...
						case "protect":
						if(!isset($args[1])){
							$sender->sendMessage("§a【運営】 §cⓘ保護モードが0か1か2か選択してください");
						}else{
							switch($args[1]){
								case "0":
								$this->setting->set("protectnotice",0);
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ドア設置時の保護モードは0になりました");
								break;
								
								case "1":
								$this->setting->set("protectnotice",1);
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ドア設置時の通知は1になりました");
								break;
								
								case "2":
								$this->setting->set("protectnotice",2);
								$this->setting->save();
								$this->getServer()->broadcastMessage("§a【運営】 §fDP : ドア設置時の通知は2になりました");
								break;
								
								default:
								$sender->sendMessage("§a【運営】 §c0か1か2か選んで下さい");
								break;
							}
						}
						break;
	 ...
	 public function onBreak(BlockBreakEvent $event){
		...
		if($this->doordata->exists($breakplace2)){
				if($player->isOp()){
					$player->sendMessage("§a【運営】 §fドアは撤去され、保護が解除されました");
					...
					if($master[0] == $ip || $master[1] == $name){
						...
						$player->sendMessage("§a【運営】 §cあなたはこの家のオーナーではありません");
						$this->getServer()->broadcastMessage("§a[運営] §c" .$name ."が" .$master[1] ."の家のドアを壊そうとしました");
						$this->batu($player,$doorplace);
						$event->setCancelled();
					}
				}
		...
		if(
			...
			$doorplace = $doorx.",".$doory.",".$doorz.",".$doorworld;
				...
				switch($this->setting->get("protectnotice")){
					case 1:
					$player->sendMessage("§a【運営】 §f/dp p でドアを保護することをお勧めします");
					break;
					...
					$this->doordata->save();
					$player->sendMessage("§a【運営】 §fドアが自動保護されました");
					break;
					}else{
					$player->sendMessage("§a【運営】 §cドアの設置に失敗したので、保護はされません");	
					}
				}
	...
	public function batu($player,$doorplace){
		...
		switch($this->setting->get("penalty")){
			...
			case 1:
				$name = $player->getName();
				$this->getServer()->broadcastMessage("§a[運営] §c" .$name ."が" .$master[1] ."の家に侵入しました!");
			break;
			
			case 2:
				$name = $player->getName();
				$this->getServer()->broadcastMessage("§a[運営] §c" .$name ."が" .$master[1] ."の家に侵入しました!");
				$player->kick("§a【運営】 §c他人の家に勝手に入らないで下さい", true);
			break;
